        ...
        for operation_candidate in operation_candidates_list:
            ...
            for operation in operation_candidate:
                ...
                if operation in self.conv_candidate:
                    operation_obj = ConvPool(conv_name=operation, input_dim=self.hidden_dimension, output_dim=self.hidden_dimension).to(self.device)
                elif operation in self.norm_candidate:
                    operation_obj = NormPool(norm_name=operation, input_dim=self.hidden_dimension).to(self.device)
...
#     def __init__(self, gnn, input_dim, hidden_dim, output_dim):
#         super(GNNModule, self).__init__()
#         self.pre_mlp = MLP(input_dim, hidden_dim)
#         self.conv1 = ConvPool(hidden_dim, hidden_dim, conv_name=gnn[0])
#         self.norm1 = NormPool(hidden_dim,gnn[1])
#         self.act1 = ActPool(gnn[2])
#         self.conv2 = ConvPool(hidden_dim, hidden_dim, conv_name=gnn[3])
#         self.norm2 = NormPool(hidden_dim,gnn[4])
#         self.act2 = ActPool(gnn[5])
#         self.post_mlp = MLP(hidden_dim, output_dim)
#     def forward(self, x, edge_index):
#         x = self.pre_mlp(x)
