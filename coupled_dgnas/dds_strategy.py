...
class DDS(object):
    ...
    def search(self, graph, inner_search_epoch, return_top_k):
        ...
        return best_gnn_architecture_candidates_list, total_epoch
...
if __name__=="__main__":
    ...
    dynamic_supernet_config = {"ConvPool": {"remain_size": len(ConvPool.candidate_list)},
                                   "NormPool": {"remain_size": len(NormPool.candidate_list)},
                                   "ActPool": {"remain_size": len(ActPool.candidate_list)}}
    ...
    _, total_epoch = mydds.search(graph, 50, 5)
    time_ = perf_counter() - st
    print("DSS average epoch time {:.4f}".format(time_ / total_epoch))