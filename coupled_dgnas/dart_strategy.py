...
from coupled_dgnas.coupled_supernet import CoupledSuperNet, ConvPool, NormPool, ActPool
import utils
...
class DARTS(object):
    ...
    def best_alpha_gnn_architecture_output(self, alpha_parameters_list):
        ...
        for alpha_list, candidate_list in zip(alpha_parameters_list, self.operation_candidates_list):
            alpha_list = alpha_list.cpu().detach()
            best_index = alpha_list.argmax().item()
...
if __name__=="__main__":
    # from experiment_gnn import train, infer
    ...
    operation_candidates_list = [ConvPool.candidate_list,
                                 NormPool.candidate_list,
                                 ActPool.candidate_list,
                                 ConvPool.candidate_list,
                                 NormPool.candidate_list,
                                 ActPool.candidate_list]
    ...
    archs = mydarts.search(graph, 50, 2)
    time_ = perf_counter()-st
    print("darts time {:.4f}".format(time_/50))
    # for arch in archs:
    #     print("darts Training")
    #     gnn = GNNModule(gnn=arch, input_dim=graph.num_node_features, hidden_dim=128, output_dim=graph.num_classes).to(device)
    #     optim = torch.optim.SGD(gnn.parameters(), momentum=0.9, nesterov=True)
    #     for i in range(100):
    #         train(gnn, graph.x, graph.edge_index, graph.y, graph.train_mask, optim)
    #     with torch.set_grad_enabled(False):
    #         res = infer(gnn, graph.x, graph.edge_index, graph.y, graph.test_mask, test=True)
    #         print(f'dataset {data_name} darts acc:{res["accuracy"]:.4f}')